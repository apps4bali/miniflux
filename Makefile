...
darwin-x86: generate
	@ GOOS=darwin GOARCH=386 go build -ldflags=$(LD_FLAGS) -o $(APP)-darwin-x86 main.go
...
run-migration:
	DATABASE_URL=$(DEV_DB_URL) go run main.go -migrate

create-admin:
	DATABASE_URL=$(DEV_DB_URL) ADMIN_USERNAME=admin ADMIN_PASSWORD=password go run main.go -create-admin

integration-test:
	psql -U postgres -c 'drop database if exists miniflux_test;'
	psql -U postgres -c 'create database miniflux_test;'
	DATABASE_URL=$(TEST_DB_URL) go run main.go -migrate
	DATABASE_URL=$(TEST_DB_URL) ADMIN_USERNAME=admin ADMIN_PASSWORD=test123 go run main.go -create-admin
